...
async def test_consent_append_only(db_session):
    ...
    consent = ConsentLedger(
        id=uuid4(), subject_hash=subject_hash,
        tx_hash=f"tx-{uuid4().hex[:16]}",
        purposes=["data_processing"], media_types=["image"],
        geography_restrictions=[], signed_at=now,
        expiry=now + timedelta(days=730), signature_bytes=b"sig123",
    ...
    result = await db_session.execute(
        select(ConsentLedger).where(ConsentLedger.subject_hash == subject_hash)
    )
...
async def test_withdrawal_cascades_to_batch_rejection(db_session):
    ...
    consent = ConsentLedger(
        ...
        purposes=["data_processing"], media_types=["image"],
        geography_restrictions=[], signed_at=now,
        expiry=now + timedelta(days=730), signature_bytes=b"sig456",
        status=ConsentStatus.ACTIVE,
    )
    db_session.add(consent)

    node = Node(
        id=uuid4(), node_id=f"NODE-W-{uuid4().hex[:8]}", district="Lilongwe",
        latitude=-13.96, longitude=33.79, category=NodeCategory.ROAD,
        hardware_profile={}, network_config={},
        capture_schedule="*/10 * * * *", interest_classes=["person"],
        pii_mode=PIIMode.MODERATE, firmware_version="1.0.0",
        public_key=b"pk", status=NodeStatus.ONLINE, is_enabled=True,
    )
    db_session.add(node)
    await db_session.flush()

    batch = IngestionBatch(
        id=uuid4(), batch_id=f"BATCH-W-{uuid4().hex[:8]}", node_id=node.id,
        hub_id="HUB-01", event_count=1, file_size_bytes=1024,
        checksum_sha256=f"sha-{uuid4().hex[:16]}", node_signature=b"sig",
        compression_codec="h264", status=BatchStatus.INGESTED,
    ...
    ann = Annotation(
        ...
        image_path="/data/test.jpg", thumbnail_path="/data/thumb.jpg",
        status=AnnotationStatus.CERTIFIED,
        detected_objects={}, auto_labels={}, quality_score=0.9,
    )
    db_session.add(ann)
    await db_session.flush()
...
async def test_buyer_notification_on_withdrawal(db_session):
    ...
    consent = ConsentLedger(
        id=uuid4(), subject_hash=subject_hash,
        tx_hash=f"tx-{uuid4().hex[:16]}",
        purposes=["marketing"], media_types=["video"],
        geography_restrictions=[], signed_at=now,
        expiry=now + timedelta(days=730), signature_bytes=b"sig789",
        status=ConsentStatus.ACTIVE,
    )
    db_session.add(consent)
    await db_session.commit()
...
async def test_pii_audit_finds_faces(db_session):
    ...
    ds = Dataset(
        id=uuid4(), dataset_id=f"ds-pii-{uuid4().hex[:8]}",
        name="PII Test", version="1.0",
        status=DatasetStatus.READY, sample_count=1,
        classes={"person": 1}, annotations_per_image=1.0,
        image_width=1920, image_height=1080,
        geographic_coverage={"countries": ["MW"]},
        demographic_report={}, consent_coverage_pct=1.0,
        pii_scrub_verified=False, iaa_score=0.9,
        formats=["COCO"], price_usd=Decimal("10.00"),
    ...
    node = Node(
        id=uuid4(), node_id=f"NODE-PII-{uuid4().hex[:8]}", district="Lilongwe",
        latitude=-13.96, longitude=33.79, category=NodeCategory.ROAD,
        hardware_profile={"gpu": "orin"}, network_config={"type": "4g"},
        capture_schedule="*/10 * * * *", interest_classes=["car", "person"],
        pii_mode=PIIMode.MODERATE, firmware_version="1.0.0",
        public_key=b"pk", status=NodeStatus.ONLINE, is_enabled=True,
    )
    db_session.add(node)
    ...
    batch = IngestionBatch(
        id=uuid4(), batch_id=f"BATCH-PII-{uuid4().hex[:8]}", node_id=node.id,
        hub_id="HUB-01", event_count=1, file_size_bytes=1024,
        checksum_sha256=f"sha-{uuid4().hex[:16]}", node_signature=b"sig",
        compression_codec="h264", status=BatchStatus.INGESTED,
        quality_scores={"overall": 0.9},
...
async def test_pii_audit_passes_clean_image(db_session):
    ...
    ds = Dataset(
        id=uuid4(), dataset_id=f"ds-clean-{uuid4().hex[:8]}",
        name="Clean Test", version="1.0",
        status=DatasetStatus.READY, sample_count=1,
        classes={"car": 1}, annotations_per_image=1.0,
        image_width=1920, image_height=1080,
        geographic_coverage={"countries": ["MW"]},
        demographic_report={}, consent_coverage_pct=1.0,
        pii_scrub_verified=True, iaa_score=0.9,
        formats=["COCO"], price_usd=Decimal("10.00"),
        license_type=LicenseType.ANNUAL,
    )
    db_session.add(ds)
    ...
    node = Node(
        id=uuid4(), node_id=f"NODE-PII-{uuid4().hex[:8]}", district="Blantyre",
        latitude=-15.79, longitude=35.00, category=NodeCategory.ROAD,
        hardware_profile={"gpu": "orin"}, network_config={"type": "4g"},
        capture_schedule="*/10 * * * *", interest_classes=["car"],
        pii_mode=PIIMode.MODERATE, firmware_version="1.0.0",
        public_key=b"pk", status=NodeStatus.ONLINE, is_enabled=True,
    )
    db_session.add(node)
