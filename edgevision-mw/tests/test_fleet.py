...
async def test_heartbeat_updates_node_status(db_session):
    ...
    hb_result = await db_session.execute(select(Heartbeat).where(Heartbeat.node_id == node.id))
...
async def test_invalid_signature_rejected(db_session):
    ...
    await db_session.execute(select(Node).where(Node.id == node.id))
    ...
    await db_session.execute(update(Node).where(Node.id == node.id).values(public_key=correct_pub_bytes))
...
async def test_offline_node_alerted(db_session):
    ...
    await db_session.execute(update(Node).where(Node.id == node.id).values(last_heartbeat_at=seven_hours_ago))
    hb = Heartbeat(
        node_id=node.id,
...
async def test_command_persistence(db_session):
    node = await _create_node(db_session)
    success = await send_node_command(db_session, node.node_id, {"command": "reboot", "payload": {"force": True}})
    ...
    assert (
        "_pending_commands" not in cleared.network_config or cleared.network_config.get("_pending_commands", []) == []
    )
...
async def test_low_battery_sets_degraded_status(db_session):
    ...
    result = await db_session.execute(select(Node).where(Node.id == node.id))
    updated_node = result.scalar_one()
    assert updated_node.status == NodeStatus.DEGRADED, f"Expected DEGRADED for low battery, got {updated_node.status}"
...
async def test_heartbeat_timeout_marks_offline(db_session):
    ...
    await db_session.execute(update(Node).where(Node.id == node.id).values(last_heartbeat_at=stale_time))
    await db_session.commit()
    ...
    result = await db_session.execute(select(Node).where(Node.id == node.id))
    updated_node = result.scalar_one()
    assert updated_node.status == NodeStatus.OFFLINE, f"Expected OFFLINE after timeout, got {updated_node.status}"
