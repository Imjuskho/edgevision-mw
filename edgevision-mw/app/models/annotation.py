...
class Annotation(StorageKeyMixin, TimestampMixin, Base):
    ...
    human_labels: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )
    qa_labels: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )
    status: Mapped[AnnotationStatus] = mapped_column(
        String(20), nullable=False, default=AnnotationStatus.PENDING
    )
    quality_score: Mapped[float] = mapped_column(Float, nullable=False)
    iaa_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    annotator_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
    qa_reviewer_id: Mapped[UUID | None] = mapped_column(
        ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
