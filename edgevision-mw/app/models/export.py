...
class Export(TimestampMixin, Base):
    ...
    dataset_id: Mapped[UUID] = mapped_column(ForeignKey("datasets.id", ondelete="CASCADE"), index=True, nullable=False)
    buyer_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True, nullable=False)
    ...
    watermark_fingerprint: Mapped[str] = mapped_column(String(128), nullable=False)
    formats_delivered: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False)
...
class ExportLog(TimestampMixin, Base):
    ...
    export_id: Mapped[UUID] = mapped_column(ForeignKey("exports.id", ondelete="CASCADE"), index=True, nullable=False)
