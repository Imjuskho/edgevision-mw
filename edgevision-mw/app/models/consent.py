...
class ConsentLedger(AppendOnlyMixin, Base):
    ...
    purposes: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False)
    media_types: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False)
    geography_restrictions: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False)
    signed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    expiry: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    guardian_hash: Mapped[str | None] = mapped_column(String(64), nullable=True)
