...
class Dataset(TimestampMixin, Base):
    ...
    status: Mapped[DatasetStatus] = mapped_column(
        String(20), nullable=False, default=DatasetStatus.BUILDING
    )
    sample_count: Mapped[int] = mapped_column(Integer, nullable=False)
    classes: Mapped[dict] = mapped_column(JSONB, nullable=False)
    annotations_per_image: Mapped[float] = mapped_column(
        Float, nullable=False
    )
    image_width: Mapped[int] = mapped_column(Integer, nullable=False)
    image_height: Mapped[int] = mapped_column(Integer, nullable=False)
    geographic_coverage: Mapped[dict] = mapped_column(
        JSONB, nullable=False
    )
    demographic_report: Mapped[dict] = mapped_column(
        JSONB, nullable=False
    )
    consent_coverage_pct: Mapped[float] = mapped_column(
        Float, nullable=False
    )
    pii_scrub_verified: Mapped[bool] = mapped_column(
    ...
    buyer_fingerprint: Mapped[str | None] = mapped_column(
